...
class AlexNet(BaseClassifier):
    def __init__(self, output_path, dataset, input_shape=200, epochs=50,
        ...
        super(AlexNet, self).__init__(output_path, dataset,
                                  fold=fold,
                                  )
    ...
    def architecture_definition(self):
        ...
        # -- first layer
        conv2d_1 = Conv2D(filters=96, kernel_size=(11,11), strides=(4, 4), padding='valid', activation='relu', name='conv_1')(img_input)
        max_pooling_1 = MaxPooling2D(pool_size=(3, 3), strides=(2,2), name='pool_1')(conv2d_1)
        # -- second layer
        conv2d_2 = Conv2D(filters=256, kernel_size=(5, 5), strides=(1, 1), padding='same', activation='relu', name='conv_2')(max_pooling_1)
        max_pooling_2 = MaxPooling2D(pool_size=(3, 3), strides=(2, 2), name='pool_2')(conv2d_2)

        conv2d_3 = Conv2D(filters=384, kernel_size=(3,3), padding='same', activation='relu', name='conv_3')(conv2d_2)
        conv2d_4 = Conv2D(filters=384, kernel_size=(3,3), padding='same', activation='relu', name='conv_4')(conv2d_3)
        conv2d_5 = Conv2D(filters=256, kernel_size=(3,3), padding='same', activation='relu', name='conv_5')(conv2d_4)
        max_pooling_3 = MaxPooling2D(pool_size=(3,3), strides=(2,2))(conv2d_5)

        # -- fully-connected layer
        flatten_1 = Flatten(name='flatten')(max_pooling_3)
