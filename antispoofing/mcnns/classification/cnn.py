...
from antispoofing.mcnns.utils.misc import *
...
class CNN(BaseClassifier):
    def __init__(self, output_path, dataset, input_shape=200, epochs=50,
                 batch_size=8, loss_function='categorical_crossentropy', lr=0.01, decay=0.0005, 
                 optimizer='SGD', regularization=0.1,
                 device_number=0, force_train=False, filter_vis=False, layers_name=('conv_1',),
                 ksize=(3,3),
                 fold=0):

        super(CNN, self).__init__(output_path, dataset,
                                  fold=fold,
                                  )
    ...
    def architecture_definition(self):
        ...
        # -- first layer
        conv2d_1 = Conv2D(filters=16, kernel_size=self.ksize, strides=(1, 1), activation='relu', name='conv_1')(img_input)
    ...
    def predict(self, x_values):
        ...
        self.prediction_time = elapsed.seconds / x_values.shape[0]
