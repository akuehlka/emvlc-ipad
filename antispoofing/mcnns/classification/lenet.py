...
class LeNet(BaseClassifier):
    def __init__(self, output_path, dataset, input_shape=200, epochs=50,
        ...
        super(LeNet, self).__init__(output_path, dataset,
                                  fold=fold,
                                  )
    ...
    def architecture_definition(self):
        ...
        # -- first layer
        conv2d_1 = Conv2D(filters=32, kernel_size=(5,5), strides=(1, 1), activation='relu', name='conv_1')(img_input)
        max_pooling_1 = MaxPooling2D(pool_size=(2, 2), name='pool_1')(conv2d_1)
        # -- second layer
        conv2d_2 = Conv2D(filters=64, kernel_size=(5, 5), strides=(1, 1), activation='relu', name='conv_2')(max_pooling_1)
        max_pooling_2 = MaxPooling2D(pool_size=(2, 2), name='pool_2')(conv2d_2)
        # -- fully-connected layer
        flatten_1 = Flatten(name='flatten')(max_pooling_2)
        dense_1 = Dense(units=100, activation='relu', name='fc1')(flatten_1)
