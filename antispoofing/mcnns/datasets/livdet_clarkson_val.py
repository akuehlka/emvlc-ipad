...
class LivDetClarkson(Dataset):
    '''
    Clarkson dataset
    '''
    def __init__(self, dataset_path, ground_truth_path='', permutation_path='', iris_location='',
                 output_path='./working', file_types=('.png', '.bmp', '.jpg', '.JPG', '.tiff'),
                 operation='crop', max_axis=320, augmentation=False, transform_vgg=False
                 ):

        super(LivDetClarkson, self).__init__(dataset_path, output_path, iris_location, file_types,
                                             operation, max_axis, augmentation=augmentation, transform_vgg=transform_vgg)

        self.ground_truth_path = ground_truth_path
        self.LIV_DET_TRAIN = os.path.join(self.ground_truth_path, 'trainset.csv')
        self.LIV_DET_TEST = os.path.join(self.ground_truth_path, 'testset.csv')
        self.LIV_DET_UNKNOWN_TEST = os.path.join(self.ground_truth_path, 'unknowntestset.csv')
        self.verbose = True
